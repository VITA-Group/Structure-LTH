	...
	for (int b=start; b<interm1; b+=8) {
		if (((b - start) & 31) == 0) {
			...
			if (threadIdx.x < end - b) {
				kernel_off = x1 + kernel_offset[threadIdx.x+b] / (256 * 3)  *65 * 256 * 64 + kernel_offset[threadIdx.x+b] / 256 % 3 * 256 * 64   + kernel_offset[threadIdx.x+b] % 256 * 64;
#pragma unroll
				for (int k=0; k<32; k++) {
...
#pragma unroll
	...
	if (interm1 < end && ((interm1-start)  & 31) == 0) {
		...
		if (threadIdx.x < end - interm1) {
			kernel_off = x1 + kernel_offset[threadIdx.x+interm1] / (256 * 3)  *65 * 256 * 64 + kernel_offset[threadIdx.x+interm1] / 256 % 3 * 256 * 64   + kernel_offset[threadIdx.x+interm1] % 256 * 64;
#pragma unroll
			for (int k=0; k<32; k++) {
