...
void _spmm_conv_0(const float * __restrict__ input_data, float *output_data, const int ptr_start, const int ptr_end, const int * __restrict__ kernel_ptr_all, const int * __restrict__ kernel_map_all, const int * __restrict__ kernel_offset, const float * __restrict__ kernel_data) {
	...
	int output_x = i / (127 * 32);
	int output_y = i /32 % 127;

	int x1 = output_x * 1 * 127 * 256 * 64 + output_y * 1 * 256 * 64 + c;
	...
	for (int b=start; b<interm1; b+=8) {
		if (((b - start) & 31) == 0) {
			...
			if (threadIdx.x < end - b) {
				kernel_off = x1 + kernel_offset[threadIdx.x+b] / (256 * 3)  *127 * 256 * 64 + kernel_offset[threadIdx.x+b] / 256 % 3 * 256 * 64   + kernel_offset[threadIdx.x+b] % 256 * 64;
#pragma unroll
				for (int k=0; k<32; k++) {
...
#pragma unroll
	...
	if (interm1 < end && ((interm1-start)  & 31) == 0) {
		...
		if (threadIdx.x < end - interm1) {
			kernel_off = x1 + kernel_offset[threadIdx.x+interm1] / (256 * 3)  *127 * 256 * 64 + kernel_offset[threadIdx.x+interm1] / 256 % 3 * 256 * 64   + kernel_offset[threadIdx.x+interm1] % 256 * 64;
#pragma unroll
			for (int k=0; k<32; k++) {
...
#pragma unroll
	...
	int output_idx = (output_x*127*256+output_y*256)*64 + c;
#pragma unroll
	for (int k=0; k<32; k++) {
...
void _spmm_conv_1(const float * __restrict__ input_data, float *output_data, const int ptr_start, const int ptr_end, const int * __restrict__ kernel_ptr_all, const int * __restrict__ kernel_map_all, const int * __restrict__ kernel_offset, const float * __restrict__ kernel_data) {
	...
	int i = (threadIdx.y * 3) + blockIdx.x * (3 << 2);
	int c = threadIdx.x + blockIdx.y * 64;
	...
	int output_x = i / (127 * 3);
	int output_y = i /3 % 127;

	int x1 = output_x * 1 * 127 * 256 * 64 + output_y * 1 * 256 * 64 + c;

	float res[3<<1];
#pragma unroll
	for (int i=0; i<(3<<1); i++) res[i] = 0.0f;

	int kernel_off;
	float kernel_value[3];
	int begin = 0;
...
void _spmm_conv_8(const float * __restrict__ input_data, float *output_data, const int ptr_start, const int ptr_end, const int * __restrict__ kernel_ptr_all, const int * __restrict__ kernel_map_all, const int * __restrict__ kernel_offset, const float * __restrict__ kernel_data) {
	...
	int i = (threadIdx.y * 29) + blockIdx.x * (29 << 2);
	int c = threadIdx.x + blockIdx.y * 64;
	...
	int output_x = i / (127 * 29);
	int output_y = i /29 % 127;

	int x1 = output_x * 1 * 127 * 256 * 64 + output_y * 1 * 256 * 64 + c;

	float res[29<<1];
#pragma unroll
	for (int i=0; i<(29<<1); i++) res[i] = 0.0f;

	int kernel_off;
	float kernel_value[29];
	int begin = 0;
...
dim3 nblocks_0(4032, 1);
dim3 nthreads_0(32, 4);
_spmm_conv_0<<<nblocks_0, nthreads_0, 0, stream_0>>>(input_data, output_data, 0, 33, kernel_ptr, kernel_map, kernel_offset, kernel_data);
cudaStreamCreate(&stream_1);
dim3 nblocks_1(4032, 1);
dim3 nthreads_1(32, 4);
_spmm_conv_1<<<nblocks_1, nthreads_1, 0, stream_1>>>(input_data, output_data, 33, 37, kernel_ptr, kernel_map, kernel_offset, kernel_data);
cudaStreamCreate(&stream_2);
dim3 nblocks_2(4032, 1);
dim3 nthreads_2(32, 4);
_spmm_conv_2<<<nblocks_2, nthreads_2, 0, stream_2>>>(input_data, output_data, 37, 70, kernel_ptr, kernel_map, kernel_offset, kernel_data);
cudaStreamCreate(&stream_3);
dim3 nblocks_3(4032, 1);
dim3 nthreads_3(32, 4);
_spmm_conv_3<<<nblocks_3, nthreads_3, 0, stream_3>>>(input_data, output_data, 70, 74, kernel_ptr, kernel_map, kernel_offset, kernel_data);
cudaStreamCreate(&stream_4);
dim3 nblocks_4(4032, 1);
dim3 nthreads_4(32, 4);
_spmm_conv_4<<<nblocks_4, nthreads_4, 0, stream_4>>>(input_data, output_data, 74, 107, kernel_ptr, kernel_map, kernel_offset, kernel_data);
cudaStreamCreate(&stream_5);
dim3 nblocks_5(4032, 1);
dim3 nthreads_5(32, 4);
_spmm_conv_5<<<nblocks_5, nthreads_5, 0, stream_5>>>(input_data, output_data, 107, 111, kernel_ptr, kernel_map, kernel_offset, kernel_data);
cudaStreamCreate(&stream_6);
dim3 nblocks_6(4032, 1);
dim3 nthreads_6(32, 4);
_spmm_conv_6<<<nblocks_6, nthreads_6, 0, stream_6>>>(input_data, output_data, 111, 144, kernel_ptr, kernel_map, kernel_offset, kernel_data);
cudaStreamCreate(&stream_7);
dim3 nblocks_7(4032, 1);
dim3 nthreads_7(32, 4);
_spmm_conv_7<<<nblocks_7, nthreads_7, 0, stream_7>>>(input_data, output_data, 144, 148, kernel_ptr, kernel_map, kernel_offset, kernel_data);
cudaStreamCreate(&stream_8);
dim3 nblocks_8(4032, 1);
dim3 nthreads_8(32, 4);
_spmm_conv_8<<<nblocks_8, nthreads_8, 0, stream_8>>>(input_data, output_data, 148, 178, kernel_ptr, kernel_map, kernel_offset, kernel_data);
cudaStreamCreate(&stream_9);
dim3 nblocks_9(4032, 1);
dim3 nthreads_9(32, 4);
_spmm_conv_9<<<nblocks_9, nthreads_9, 0, stream_9>>>(input_data, output_data, 178, 208, kernel_ptr, kernel_map, kernel_offset, kernel_data);
cudaStreamCreate(&stream_10);
dim3 nblocks_10(4032, 1);
dim3 nthreads_10(32, 4);
_spmm_conv_10<<<nblocks_10, nthreads_10, 0, stream_10>>>(input_data, output_data, 208, 238, kernel_ptr, kernel_map, kernel_offset, kernel_data);
cudaStreamCreate(&stream_11);
dim3 nblocks_11(4032, 1);
dim3 nthreads_11(32, 4);
_spmm_conv_11<<<nblocks_11, nthreads_11, 0, stream_11>>>(input_data, output_data, 238, 268, kernel_ptr, kernel_map, kernel_offset, kernel_data);
