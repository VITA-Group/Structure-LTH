...
void _spmm_conv_0(const float * __restrict__ input_data, float *output_data, const int ptr_start, const int ptr_end, const int * __restrict__ kernel_ptr_all, const int * __restrict__ kernel_map_all, const int * __restrict__ kernel_offset, const float * __restrict__ kernel_data) {
	...
	int x1 = output_x * 1 * 16 * 64 * 64 + output_y * 1 * 64 * 64 + c;
	...
	for (int b=start; b<interm1; b+=8) {
		if (((b - start) & 31) == 0) {
			...
			if (threadIdx.x < end - b) {
				kernel_off = x1 + kernel_offset[threadIdx.x+b] / (64 * 3)  *16 * 64 * 64 + kernel_offset[threadIdx.x+b] / 64 % 3 * 64 * 64   + kernel_offset[threadIdx.x+b] % 64 * 64;
#pragma unroll
				for (int k=0; k<32; k++) {
...
#pragma unroll
	...
	if (interm1 < end && ((interm1-start)  & 31) == 0) {
		...
		if (threadIdx.x < end - interm1) {
			kernel_off = x1 + kernel_offset[threadIdx.x+interm1] / (64 * 3)  *16 * 64 * 64 + kernel_offset[threadIdx.x+interm1] / 64 % 3 * 64 * 64   + kernel_offset[threadIdx.x+interm1] % 64 * 64;
#pragma unroll
			for (int k=0; k<32; k++) {
