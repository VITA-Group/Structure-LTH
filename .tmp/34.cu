...
void _spmm_conv_0(const float * __restrict__ input_data, float *output_data, const int ptr_start, const int ptr_end, const int * __restrict__ kernel_ptr_all, const int * __restrict__ kernel_map_all, const int * __restrict__ kernel_offset, const float * __restrict__ kernel_data) {
	...
	int x1 = output_x * 1 * 2 * 512 * 64 + output_y * 1 * 512 * 64 + c;
	...
	for (int b=start; b<interm1; b+=8) {
		if (((b - start) & 31) == 0) {
			...
			if (threadIdx.x < end - b) {
				kernel_off = x1 + kernel_offset[threadIdx.x+b] / (512 * 3)  *2 * 512 * 64 + kernel_offset[threadIdx.x+b] / 512 % 3 * 512 * 64   + kernel_offset[threadIdx.x+b] % 512 * 64;
#pragma unroll
				for (int k=0; k<32; k++) {
...
#pragma unroll
	...
	if (interm1 < end && ((interm1-start)  & 31) == 0) {
		...
		if (threadIdx.x < end - interm1) {
			kernel_off = x1 + kernel_offset[threadIdx.x+interm1] / (512 * 3)  *2 * 512 * 64 + kernel_offset[threadIdx.x+interm1] / 512 % 3 * 512 * 64   + kernel_offset[threadIdx.x+interm1] % 512 * 64;
#pragma unroll
			for (int k=0; k<32; k++) {
...
#pragma unroll
	...
	int output_idx = (output_x*2*512+output_y*512)*64 + c;
#pragma unroll
	for (int k=0; k<32; k++) {
...
void _spmm_conv_1(const float * __restrict__ input_data, float *output_data, const int ptr_start, const int ptr_end, const int * __restrict__ kernel_ptr_all, const int * __restrict__ kernel_map_all, const int * __restrict__ kernel_offset, const float * __restrict__ kernel_data) {
	...
	int i = (threadIdx.y * 14) + blockIdx.x * (14 << 2);
	int c = threadIdx.x + blockIdx.y * 64;
	...
	float res[14<<1];
#pragma unroll
	for (int i=0; i<(14<<1); i++) res[i] = 0.0f;

	int kernel_off;
	float kernel_value[14];
	int begin = 0;
...
void _spmm_conv_5(const float * __restrict__ input_data, float *output_data, const int ptr_start, const int ptr_end, const int * __restrict__ kernel_ptr_all, const int * __restrict__ kernel_map_all, const int * __restrict__ kernel_offset, const float * __restrict__ kernel_data) {
	...
	int i = (threadIdx.y * 28) + blockIdx.x * (28 << 2);
	int c = threadIdx.x + blockIdx.y * 64;
	...
	float res[28<<1];
#pragma unroll
	for (int i=0; i<(28<<1); i++) res[i] = 0.0f;

	int kernel_off;
	float kernel_value[28];
	int begin = 0;
...
void _spmm_conv_7(const float * __restrict__ input_data, float *output_data, const int ptr_start, const int ptr_end, const int * __restrict__ kernel_ptr_all, const int * __restrict__ kernel_map_all, const int * __restrict__ kernel_offset, const float * __restrict__ kernel_data) {
	...
	int i = (threadIdx.y * 19) + blockIdx.x * (19 << 2);
	int c = threadIdx.x + blockIdx.y * 64;
	...
	float res[19<<1];
#pragma unroll
	for (int i=0; i<(19<<1); i++) res[i] = 0.0f;

	int kernel_off;
	float kernel_value[19];
	int begin = 0;
...
void _spmm_conv_11(const float * __restrict__ input_data, float *output_data, const int ptr_start, const int ptr_end, const int * __restrict__ kernel_ptr_all, const int * __restrict__ kernel_map_all, const int * __restrict__ kernel_offset, const float * __restrict__ kernel_data) {
	...
	int i = (threadIdx.y * 5) + blockIdx.x * (5 << 2);
	int c = threadIdx.x + blockIdx.y * 64;
	...
	float res[5<<1];
#pragma unroll
	for (int i=0; i<(5<<1); i++) res[i] = 0.0f;

	int kernel_off;
	float kernel_value[5];
	int begin = 0;
...
_spmm_conv_1<<<nblocks_1, nthreads_1, 0, stream_1>>>(input_data, output_data, 97, 112, kernel_ptr, kernel_map, kernel_offset, kernel_data);
cudaStreamCreate(&stream_2);
dim3 nblocks_2(3, 1);
dim3 nthreads_2(32, 4);
_spmm_conv_2<<<nblocks_2, nthreads_2, 0, stream_2>>>(input_data, output_data, 112, 209, kernel_ptr, kernel_map, kernel_offset, kernel_data);
cudaStreamCreate(&stream_3);
dim3 nblocks_3(1, 1);
dim3 nthreads_3(32, 4);
_spmm_conv_3<<<nblocks_3, nthreads_3, 0, stream_3>>>(input_data, output_data, 209, 224, kernel_ptr, kernel_map, kernel_offset, kernel_data);
cudaStreamCreate(&stream_4);
dim3 nblocks_4(2, 1);
dim3 nthreads_4(32, 4);
_spmm_conv_4<<<nblocks_4, nthreads_4, 0, stream_4>>>(input_data, output_data, 224, 289, kernel_ptr, kernel_map, kernel_offset, kernel_data);
cudaStreamCreate(&stream_5);
dim3 nblocks_5(1, 1);
dim3 nthreads_5(32, 4);
_spmm_conv_5<<<nblocks_5, nthreads_5, 0, stream_5>>>(input_data, output_data, 289, 318, kernel_ptr, kernel_map, kernel_offset, kernel_data);
cudaStreamCreate(&stream_6);
dim3 nblocks_6(1, 1);
dim3 nthreads_6(32, 4);
_spmm_conv_6<<<nblocks_6, nthreads_6, 0, stream_6>>>(input_data, output_data, 318, 351, kernel_ptr, kernel_map, kernel_offset, kernel_data);
cudaStreamCreate(&stream_7);
dim3 nblocks_7(1, 1);
dim3 nthreads_7(32, 4);
_spmm_conv_7<<<nblocks_7, nthreads_7, 0, stream_7>>>(input_data, output_data, 351, 371, kernel_ptr, kernel_map, kernel_offset, kernel_data);
cudaStreamCreate(&stream_8);
dim3 nblocks_8(1, 1);
dim3 nthreads_8(32, 4);
_spmm_conv_8<<<nblocks_8, nthreads_8, 0, stream_8>>>(input_data, output_data, 371, 404, kernel_ptr, kernel_map, kernel_offset, kernel_data);
cudaStreamCreate(&stream_9);
dim3 nblocks_9(1, 1);
dim3 nthreads_9(32, 4);
_spmm_conv_9<<<nblocks_9, nthreads_9, 0, stream_9>>>(input_data, output_data, 404, 411, kernel_ptr, kernel_map, kernel_offset, kernel_data);
cudaStreamCreate(&stream_10);
dim3 nblocks_10(1, 1);
dim3 nthreads_10(32, 4);
_spmm_conv_10<<<nblocks_10, nthreads_10, 0, stream_10>>>(input_data, output_data, 411, 444, kernel_ptr, kernel_map, kernel_offset, kernel_data);
cudaStreamCreate(&stream_11);
dim3 nblocks_11(1, 1);
dim3 nthreads_11(32, 4);
_spmm_conv_11<<<nblocks_11, nthreads_11, 0, stream_11>>>(input_data, output_data, 444, 450, kernel_ptr, kernel_map, kernel_offset, kernel_data);
cudaStreamCreate(&stream_12);
dim3 nblocks_12(1, 1);
dim3 nthreads_12(32, 4);
_spmm_conv_12<<<nblocks_12, nthreads_12, 0, stream_12>>>(input_data, output_data, 450, 483, kernel_ptr, kernel_map, kernel_offset, kernel_data);
cudaStreamCreate(&stream_13);
dim3 nblocks_13(1, 1);
dim3 nthreads_13(32, 4);
_spmm_conv_13<<<nblocks_13, nthreads_13, 0, stream_13>>>(input_data, output_data, 483, 489, kernel_ptr, kernel_map, kernel_offset, kernel_data);
cudaStreamCreate(&stream_14);
dim3 nblocks_14(1, 1);
dim3 nthreads_14(32, 4);
_spmm_conv_14<<<nblocks_14, nthreads_14, 0, stream_14>>>(input_data, output_data, 489, 522, kernel_ptr, kernel_map, kernel_offset, kernel_data);
cudaStreamCreate(&stream_15);
dim3 nblocks_15(1, 1);
dim3 nthreads_15(32, 4);
_spmm_conv_15<<<nblocks_15, nthreads_15, 0, stream_15>>>(input_data, output_data, 522, 528, kernel_ptr, kernel_map, kernel_offset, kernel_data);
