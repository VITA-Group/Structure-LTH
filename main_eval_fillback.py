...
parser.add_argument('--criterion', default="remain", type=str, choices=['remain', 'magnitude', 'l1', 'l2', 'saliency'])
parser.add_argument('--fillback-rate', default=0.0, type=float)
...
def load_ticket(model, args, train_loader=None):
    ...
    if args.mask_dir:
        ...
        if args.reinit:
            checkpoint['init_weight'] = torch.load(args.reinit, map_location=torch.device('cuda:'+str(args.gpu)))
